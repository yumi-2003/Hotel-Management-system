...
import { 
  DollarSign, Calendar, Home, Users, 
  Settings, Key, ClipboardList, BarChart3, UserPlus, Loader2
} from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-[#0F2F2F]">Manager Dashboard</h1>
          <p className="text-muted-foreground">Operational overview for {user?.fullName || 'Manager'}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white border border-border rounded-3xl p-8 shadow-sm">
             <h2 className="text-xl font-bold text-[#0F2F2F] mb-6">Today's Arrivals</h2>
             <div className="space-y-4">
                {todaysArrivals.length === 0 ? (
                  <div className="py-8 text-center text-muted-foreground">No arrivals expected today.</div>
                ) : (
                  todaysArrivals.map((booking: any) => (
                    <div key={booking._id} className="flex items-center justify-between p-4 rounded-xl border border-border bg-slate-50/30">
                       <div>
                          <div className="font-bold text-[#0F2F2F]">{booking.guestId?.fullName || 'Guest'}</div>
                          <div className="text-xs text-muted-foreground">Room: {booking.bookedRooms?.map((r: any) => r.roomId?.roomNumber).join(', ') || 'No room assigned'}</div>
                       </div>
                       <div className="flex gap-2">
                          {(booking.status === 'confirmed' || booking.status === 'confirmed_unpaid') ? (
                            ...
                            >
                              {processingId === booking._id ? <Loader2 size={12} className="animate-spin" /> : 'Check In'}
                            </Button>
                          ) : (
                            <span className="text-[10px] font-black uppercase text-spa-teal bg-spa-teal/10 px-2 py-1 rounded">{booking.status.replace('_', ' ')}</span>
                          )}
                        </div>
                    </div>
                  ))
          ...
          <div className="bg-white border border-border rounded-3xl p-8 shadow-sm">
             <h2 className="text-xl font-bold text-[#0F2F2F] mb-6">Today's Departures</h2>
             <div className="space-y-4">
                {todaysDepartures.length === 0 ? (
                  <div className="py-8 text-center text-muted-foreground">No departures today.</div>
                ) : (
                  todaysDepartures.map((booking: any) => (
                    <div key={booking._id} className="flex items-center justify-between p-4 rounded-xl border border-border bg-slate-50/30">
                       <div>
                          <div className="font-bold text-[#0F2F2F]">{booking.guestId?.fullName || 'Guest'}</div>
                          <div className="text-xs text-muted-foreground">Room: {booking.bookedRooms?.map((r: any) => r.roomId?.roomNumber).join(', ') || 'Room unknown'}</div>
                       </div>
                       <div className="flex gap-2">
                          {booking.status === 'checked_in' && (
                            ...
                            >
                              {processingId === booking._id ? <Loader2 size={12} className="animate-spin" /> : 'Check Out'}
                            </Button>
                          )}
                          <Button size="sm" variant="outline" onClick={() => navigate(`/staff/bookings`)} className="text-[10px] h-8 px-3">Details</Button>
                        </div>
                    </div>
                  ))
                )}
      ...
      <div className="bg-white border border-border rounded-3xl p-8 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Link to="/admin/rooms" className="group p-6 rounded-2xl border border-border hover:border-spa-teal hover:bg-spa-teal/[0.02] transition-all">
            ...
            <p className="text-xs text-muted-foreground mt-1">Configure room types & pricing</p>
          </Link>

          <Link to="/admin/amenities" className="group p-6 rounded-2xl border border-border hover:border-spa-teal hover:bg-spa-teal/[0.02] transition-all">
            ...
            <p className="text-xs text-muted-foreground mt-1">Manage global hotel features</p>
          </Link>

          <Link to="/staff/reservations" className="group p-6 rounded-2xl border border-border hover:border-spa-teal hover:bg-spa-teal/[0.02] transition-all">
            ...
            <p className="text-xs text-muted-foreground mt-1">Review guest bookings</p>
          </Link>

          <Link to="/staff/bookings" className="group p-6 rounded-2xl border border-border hover:border-spa-teal hover:bg-spa-teal/[0.02] transition-all">
            ...
            <p className="text-xs text-muted-foreground mt-1">Manage check-ins & check-outs</p>
          </Link>

          <Link to="/admin/users" className="group p-6 rounded-2xl border border-border hover:border-spa-teal hover:bg-spa-teal/[0.02] transition-all">
            ...
            <p className="text-xs text-muted-foreground mt-1">Create & manage staff accounts</p>
          </Link>

          <Link to="/manager/reports" className="group p-6 rounded-2xl border border-border hover:border-spa-teal hover:bg-spa-teal/[0.02] transition-all">
            ...
            <p className="text-xs text-muted-foreground mt-1">Analyze revenue & occupancy</p>
          </Link>
        </div>
      </div>
    </div>
