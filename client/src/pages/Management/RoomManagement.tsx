...
import { getAllRooms, updateRoomStatus, createRoom } from '../../services/individualRoomService';
import { getAllRoomTypes } from '../../services/roomService';
import type { Room, RoomType } from '../../types';
import { 
  Plus, CheckCircle2, Wrench, 
  Search, Loader2, Key, Info, Layers, Clock
} from 'lucide-react';
...
const RoomManagement = () => {
  ...
  const handleAddRoom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewRoom({ roomNumber: '', roomType: roomTypes[0]?._id || '', floor: 1 });
      toast.success('Room added successfully!');
    } catch (error) {
      console.error('Failed to add room', error);
      toast.error('Failed to add room. Please check if room number already exists.');
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage individual rooms and their operational status</p>
        </div>
        <Button 
                ...
                filtered.map((r) => (
                  <tr key={r._id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                       <span className="font-black text-lg text-[#0F2F2F]">{r.roomNumber}</span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${getStatusStyles(r.status)}`}>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200">
             <h2 className="text-2xl font-bold text-[#0F2F2F] mb-6">Add Individual Room</h2>
             <form onSubmit={handleAddRoom} className="space-y-4">
               ...
               <div>
                 <label className="block text-sm font-bold text-muted-foreground mb-1">Floor</label>
                 <input 
                   required
                   type="number" 
                   value={newRoom.floor}
               ...
               <div className="flex justify-end gap-3 pt-4">
                 ...
                 <Button type="submit" className="bg-spa-teal text-white">Add Room</Button>
               </div>
             </form>
          </div>
        </div>
