...
const CreateStaffModal = ({
  ...
  const allowed =
    callerRole === "admin" ? ADMIN_CREATABLE_ROLES : MANAGER_CREATABLE_ROLES;
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="px-8 py-6 space-y-4">
          ...
          <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-xl border border-border">
            ...
            <span
              className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${ROLE_STYLES[form.role]}`}
            >
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button
              type="submit"
              disabled={submitting}
              className="bg-spa-teal hover:bg-spa-teal/90 text-white gap-2"
            >
              {submitting ? (
...
const UserManagement = () => {
  ...
  const handleRoleChange = async (id: string, newRole: string) => {
    try {
      ...
      setUsers(
        users.map((u) => (u._id === id ? ({ ...u, role: newRole } as any) : u)),
      );
  ...
  const staffCount = users.filter((u) =>
    ["manager", "receptionist", "housekeeping"].includes((u as any).role),
  ).length;
  const activeCount = users.filter(
    (u) => (u as any).status === "active",
  ).length;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm overflow-hidden ${ROLE_STYLES[uAny.role] || "bg-slate-100 text-slate-500"}`}
                          >
                            {uAny.profileImage ? (
                              <img
                                src={`http://localhost:4000${uAny.profileImage}`}
                      ...
                      <td className="px-6 py-4">
                        {canEdit && assignableRoles.includes(uAny.role) ? (
                          ...
                          <span
                            className={`inline-block text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${ROLE_STYLES[uAny.role] || "bg-slate-100 text-slate-500"}`}
                          >
                      ...
                      <td className="px-6 py-4">
                        <button
                          onClick={() =>
                            canEdit && handleStatusToggle(u._id, uAny.status)
                          }
                      ...
                      <td className="px-6 py-4">
                        <span className="text-xs text-muted-foreground">
                          {uAny.createdAt
                            ? new Date(uAny.createdAt).toLocaleDateString(
                                undefined,
                                {
                                  month: "short",
                                  day: "numeric",
