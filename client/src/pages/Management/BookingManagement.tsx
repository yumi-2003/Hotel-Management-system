...
const BookingManagement = () => {
  ...
  const fetchBookings = async () => {
    try {
      setLoading(true);
      const data = await getAllBookings({ status: statusFilter });
      setBookings(data);
    } catch (error) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'confirmed_unpaid': return 'bg-amber-100 text-amber-700 border-amber-200';
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage guest stays, check-ins, and departures</p>
        </div>
      </div>
                  ...
                  <tr key={b._id} className="group hover:bg-slate-50 transition-all duration-300">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-black text-[#0F2F2F] text-sm tracking-tight leading-tight mb-1">
                            {typeof b.user === 'object' ? b.user.fullName : 'Unknown Guest'}
                          </div>
                          <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                            <Mail size={10} className="text-spa-teal" /> {typeof b.user === 'object' ? b.user.email : '-'}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-xs font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Room</div>
                          <div className="font-black text-[#0F2F2F] text-base leading-none">
                            {typeof b.room === 'object' ? b.room.roomNumber : 'Unassigned'}
                          </div>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-black text-[#0F2F2F] tracking-tight">
                            {b.checkInDate ? new Date(b.checkInDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-black text-[#0F2F2F] tracking-tight">
                            {b.checkOutDate ? new Date(b.checkOutDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] border ${getStatusColor(b.status)}`}>
