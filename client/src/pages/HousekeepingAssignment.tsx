...
const HousekeepingAssignment = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8 min-h-screen bg-background text-foreground">
      <div>
        <h1 className="text-4xl font-black text-foreground tracking-tight">Assignment Command</h1>
        <p className="text-muted-foreground font-medium">Coordinate the cleanup crew and distribute daily tasks</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl font-black text-foreground uppercase tracking-wider">Unassigned Tasks</h2>
          </div>
          
          {tasks.length === 0 ? (
            <div className="bg-card border-2 border-dashed border-border rounded-[2rem] p-16 text-center">
              <ShieldCheck size={48} className="text-muted-foreground/30 mx-auto mb-4" />
              <p className="text-muted-foreground/60 font-bold">No rooms currently awaiting assignment.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {tasks.map((task) => (
                <div key={task._id} className="bg-card border border-border rounded-[2rem] p-6 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <div className="font-black text-foreground text-lg mb-1">{task.task}</div>
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      <select 
                        className="flex-1 sm:w-48 bg-muted/30 border-none rounded-xl px-4 py-3 text-sm font-bold text-foreground outline-none ring-2 ring-transparent focus:ring-spa-teal/20 transition"
                        onChange={(e) => handleAssign(task._id, e.target.value)}
                        defaultValue=""
                      >
                        <option value="" disabled className="bg-card">Select Staff</option>
                        {housekeepers.map(hk => (
                          <option key={hk._id} value={hk._id} className="bg-card">{hk.fullName}</option>
                        ))}
                      </select>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl font-black text-foreground uppercase tracking-wider">On-Duty Staff</h2>
          </div>
          <div className="bg-card border border-border rounded-[2rem] p-6 shadow-sm divide-y divide-border">
            {housekeepers.length === 0 ? (
              <p className="text-muted-foreground/60 font-bold py-4 text-center">No staff found.</p>
            ) : (
              housekeepers.map(hk => (
                <div key={hk._id} className="py-4 first:pt-0 last:pb-0 flex items-center justify-between group">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-muted rounded-full flex items-center justify-center text-muted-foreground font-black text-xs">
                      {hk.fullName.split(' ').map(n => n[0]).join('')}
                    </div>
                    <div>
                      <div className="font-bold text-foreground text-sm">{hk.fullName}</div>
                      <div className="text-[10px] text-muted-foreground uppercase font-black tracking-tighter">Housekeeping Agent</div>
                    </div>
                  </div>
                  <ArrowRight size={14} className="text-muted-foreground/30 group-hover:text-spa-teal transition-colors" />
                </div>
              ))
            )}
          </div>

          <div className="bg-[#0F2F2F] dark:bg-slate-900 border border-white/5 rounded-[2rem] p-8 text-white">
