...
const ADMIN_STAFF_ROLES = [UserRole.MANAGER, UserRole.RECEPTIONIST, UserRole.HOUSEKEEPING];
...
export const getAllUsers = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (req.user?.role === UserRole.MANAGER) {
      query.role = role && MANAGER_ALLOWED_ROLES.includes(role as UserRole)
        ? role
...
export const updateUserStatus = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const user = await User.findByIdAndUpdate(id, { status }, { returnDocument: 'after' }).select('-passwordHash');
    res.json(user);
  } catch (error) {
...
export const updateUserRole = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const user = await User.findByIdAndUpdate(id, { role }, { returnDocument: 'after' }).select('-passwordHash');
    res.json(user);
  } catch (error) {
