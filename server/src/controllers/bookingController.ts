...
export const createBooking = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    const subtotalAmount = bookedRooms.reduce((sum: number, br: any) => sum + br.subtotal, 0);
    ...
    if (reservationId) {
      await Reservation.findByIdAndUpdate(reservationId, { status: ReservationStatus.CONFIRMED }).session(session);
    }

    if (bookedRooms && bookedRooms.length > 0) {
      const roomIds = bookedRooms.map((br: any) => br.roomId);
      const newRoomStatus = (booking.status === BookingStatus.CHECKED_IN) ? RoomStatus.OCCUPIED : RoomStatus.RESERVED;
...
export const updateBookingStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (status === BookingStatus.CHECKED_OUT && oldStatus !== BookingStatus.CHECKED_OUT) {
      ...
      const HousekeepingLog = (await import('../models/HousekeepingLog')).default;
    ...
    const updatedBooking = await Booking.findById(id).populate('guestId').populate('bookedRooms.roomId');
    if (!updatedBooking) {
